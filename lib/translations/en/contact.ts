export const contact = {
  "contact.title": "Get in Touch",
  "contact.subtitle": "Have questions or suggestions? We'd love to hear from you.",
  "contact.email.label": "Email Us",
  "contact.email.description": "For general inquiries, partnership opportunities, or just to say hello:",
  "contact.email.address": "hello@nihaoholidays.org",
  "contact.response.label": "Response Time",
  "contact.response.description": "We typically respond within 24-48 hours during business days.",
  "contact.form.name": "Your Name",
  "contact.form.email": "Email Address",
  "contact.form.subject": "Subject",
  "contact.form.message": "Message",
  "contact.form.submit": "Send Message",
  "contact.form.sending": "Sending...",
  "contact.form.success": "Thank you for your message! We'll get back to you soon.",
  "contact.form.error": "Something went wrong. Please try again later.",
} as const;
