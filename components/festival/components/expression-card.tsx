  ...
  return (
    <div className="p-4 rounded-xl border border-rose-200 dark:border-rose-800 bg-white/50 dark:bg-zinc-800/50 hover:shadow-md transition-all duration-300">
      <div className="flex items-start justify-between mb-3">
        <div>
          <h4 className="font-semibold text-foreground text-lg">{expression.chinese}</h4>
          <p className="text-sm text-rose-600 dark:text-rose-400 font-medium">{expression.pinyin}</p>
        </div>
        <button
          type="button"
          onClick={speak}
      ...
      <span className="text-xs px-2 py-1 rounded-full bg-muted text-muted-foreground">{expression.scenario}</span>
    </div>
  );
}
