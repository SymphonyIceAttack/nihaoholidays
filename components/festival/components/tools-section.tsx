...
export function ToolsSection({ tools, lang, festivalId, userType }: ToolsSectionProps) {
    ...
    <div className="max-w-4xl mx-auto mt-12">
      <div className="bg-gradient-to-r from-rose-100/80 to-orange-100/80 dark:from-rose-900/30 dark:to-orange-900/20 border border-rose-200 dark:border-rose-800 rounded-2xl p-6 md:p-8">
        <div className="text-center mb-6">
          <h2 className="text-xl font-semibold text-foreground mb-2">Interactive Tools</h2>
          <p className="text-muted-foreground">Practice with scenario-based tools</p>
        </div>
        <div className="grid gap-4 md:grid-cols-3">
          {tools.map((tool, index) => {
            let href = `/${lang}/tool/menu`;

            if (tool.name.includes("Greeting")) href = `/${lang}/tool/greetings`;
            else if (tool.name.includes("Red Envelope")) href = `/${lang}/tool/red-envelope`;
            else if (tool.name.includes("Culture") || tool.name.includes("文化概览")) href = `/${lang}/culture/${festivalId}/${userType}`;
            else if (tool.name.includes("Quiz") || tool.name.includes("测验")) href = `/${lang}/quiz/${festivalId}/${userType}`;
